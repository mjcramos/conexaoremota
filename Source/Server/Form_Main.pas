...
var
  frm_Main: Tfrm_Main;
  var ID_cliente, senha_cliente, apelido : string;
...
begin
  frm_Main.Logs_Memo.Lines.Add(ptext);
end;
...
begin
  ...
  while i < frm_Main.Connections_ListView.Items.Count do
  begin
    if (frm_Main.Connections_ListView.Items.Item[i].SubItems[1] <> '') then
    begin
      llista := llista +frm_Main.Connections_ListView.Items.Item[i].SubItems[1]+' - '+
      frm_Main.Connections_ListView.Items.Item[i].SubItems[2]+' - '+
      frm_Main.Connections_ListView.Items.Item[i].SubItems[5]+',';
    end;
    Inc(i);
  end;
  result := llista;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        apelido := Copy(BufferTemp, 1, Pos('|APELIDO|>', BufferTemp) - 1);
      end;
   //   ID_cliente
...
//    ID := ID_cliente;
  ...
  L.SubItems.Add(apelido);
  L.SubItems.Add(DateToStr(now)+' - '+timetostr(now));
  L.SubItems.Objects[4] := TObject(0);
  apelido := '';
  ID_cliente := '';
  addlog('MT '+IntToStr(mainSocket.Handle)+' ID '+ID+' Senha '+password+' Apelido '+apelido);
end;
...
begin
  ...
  addlog('TThreadConnection_Main.Execute manda ID p cliente '+ID+' '+password);
  ...
  begin
    ...
    if not Buffer.Contains('<|PONG|>') then
      addlog('Buffer proc '+ID+' '+Buffer);
    position := Pos('<|GETLISTA|>', Buffer);
    if position > 0 then
    begin
      BufferTemp := Getlistaids;
      while mainSocket.SendText('<|LISTAIDS|>'+BufferTemp+'<|END|>') < 0 do
        Sleep(ProcessingSlack);
    end;
    ...
    begin
      ...
      if L2.SubItems.Objects[1] <> nil then
      ...
      if L.SubItems.Objects[1] <> nil then
      ...
      if L2.SubItems.Objects[2] <> nil then
      ...
      if L2.SubItems.Objects[3] <> nil then
    ...
    begin
      ...
      Synchronize(
        ...
        begin
          if L <> nil then
            L.SubItems[3] := '';   // aqui esta o problema
          if L2 <> nil then
            L2.SubItems[3] := '';  // aqui esta o problema
        end);
    end;
  ...
  if (targetMainSocket <> nil) and (targetMainSocket.Connected) then
  begin
    addlog('send <|DISCONNECTED|> '+ID+' '+apelido);
  ...
  Synchronize(
    procedure
    begin
      frm_Main.Logs_Memo.Lines.Add('Sincronize L del '+L.SubItems[1]+' Sub.3 '+L.SubItems[3]+' vai excluir ');
      if L.SubItems[3] <> '' then
      begin
        L2 := FindListItemID(L.SubItems[3]);
        if L2 <> nil then
        begin
          frm_Main.Logs_Memo.Lines.Add('Sincronize L2 vai limpar'+L.SubItems[3]);
          L2.SubItems[3] := '';
        end;
      end;
      L.Delete;
